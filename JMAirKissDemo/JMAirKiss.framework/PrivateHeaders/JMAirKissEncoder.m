 ...
 */
- (NSMutableArray *)createAirKissEncorderWithSSID:(NSString *) ssid withPSW:(NSString *) psw {
    ...
    while (times-- > 0) {
        [self getLeadingPart];
        [self getMagicCodeWithSSID:ssid withPSW:psw];
        
        for (int i = 0; i < 15; i++) {
            [self getPrefixCodeWithPSW:psw];
            NSMutableData *data = [NSMutableData dataWithData:[psw dataUsingEncoding:NSUTF8StringEncoding]];// 密码
            ...
            for (index = 0;index < (data.length / size); index++) {
                ...
                [self getSequenceWithIndex:index withDatas:tempData];
            }
 ...
 *  magic code
 */
- (void)getMagicCodeWithSSID:(NSString *) ssid withPSW:(NSString *) psw {
    UInt8 length = ssid.length + psw.length + 1;
 ...
 */
- (void)getSequenceWithIndex:(UInt8) index withDatas:(NSData *) data {
